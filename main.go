...
func main() {
	...
	flag.BoolVar(&auto, "auto", false, "with this option try extract the `suffix` and the `date` from the folder name")
	flag.Parse()
	...
	c, err := doFolderRename(folder, suffix, timeOffset, classify, basedir, auto)
	endTime := time.Now()
